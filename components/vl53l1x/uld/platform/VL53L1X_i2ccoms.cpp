...
int8_t i2c_write_multi(uint8_t deviceAddress, uint16_t registerAddress, uint8_t *pdata, uint32_t count) {
    return esphome::vl53l1x::vl53l1x_platform_write_multi(((deviceAddress) >> 1) & 0x7F, registerAddress, pdata, count);
}

int8_t i2c_read_multi(uint8_t deviceAddress, uint16_t registerAddress, uint8_t *pdata, uint32_t count){
    return esphome::vl53l1x::vl53l1x_platform_read_multi(((deviceAddress) >> 1) & 0x7F, registerAddress, pdata, count);
}
