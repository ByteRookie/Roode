...
roode:
  id: roode_platform
  # Optional shared secret required for portal access; omit to leave the portal open
...
| `roode.portal_password` | Optional | none | Shared token for portal/API requests | Restrict portal access on shared networks | Supply via `token` query or `X-Portal-Token` header | `portal_password: !secret portal_password` | *(not set)* |
