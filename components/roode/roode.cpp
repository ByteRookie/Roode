...
void Roode::setup() {
  ...
  if (invalid_distance_limit_number_ != nullptr)
    invalid_distance_limit_number_->publish_state(invalid_distance_limit_);
  if (restart_timeout_number_ != nullptr)
    restart_timeout_number_->publish_state(restart_timeout_ms_ / 1000);
  if (detection_min_number_ != nullptr)
    detection_min_number_->publish_state(entry->threshold->min_percentage.value_or(0));
  if (detection_max_number_ != nullptr)
    detection_max_number_->publish_state(entry->threshold->max_percentage.value_or(0));
  if (filter_mode_select_ != nullptr) {
    ...
    filter_mode_select_->publish_state(mode);
  }
  if (log_fallback_switch_ != nullptr)
    log_fallback_switch_->publish_state(log_fallback_events_);
  if (force_single_core_switch_ != nullptr)
    force_single_core_switch_->publish_state(force_single_core_);
  ...
  if (calibration_ranging_select_ != nullptr) {
    const char *mode = "auto";
    auto override = distanceSensor->get_ranging_mode_override();
    if (override.has_value())
      mode = (*override)->name;
    calibration_ranging_select_->publish_state(mode);
  }
  if (calibration_offset_number_ != nullptr && distanceSensor->offset.has_value())
    calibration_offset_number_->publish_state(*distanceSensor->offset);
  if (calibration_crosstalk_number_ != nullptr && distanceSensor->xtalk.has_value())
    calibration_crosstalk_number_->publish_state(*distanceSensor->xtalk);
  if (calibration_persistence_switch_ != nullptr)
    calibration_persistence_switch_->publish_state(calibration_persistence_);
  if (zones_invert_switch_ != nullptr)
    zones_invert_switch_->publish_state(invert_direction_);
  if (entry_roi_height_number_ != nullptr)
    entry_roi_height_number_->publish_state(entry->roi_override->height);
  if (entry_roi_center_number_ != nullptr)
    entry_roi_center_number_->publish_state(entry->roi_override->center);
  if (entry_threshold_min_number_ != nullptr)
    entry_threshold_min_number_->publish_state(entry->threshold->min_percentage.value_or(0));
  if (entry_threshold_max_number_ != nullptr)
    entry_threshold_max_number_->publish_state(entry->threshold->max_percentage.value_or(0));
  if (exit_roi_height_number_ != nullptr)
    exit_roi_height_number_->publish_state(exit->roi_override->height);
  if (exit_roi_center_number_ != nullptr)
    exit_roi_center_number_->publish_state(exit->roi_override->center);
  if (exit_threshold_min_number_ != nullptr)
    exit_threshold_min_number_->publish_state(exit->threshold->min_percentage.value_or(0));
  if (exit_threshold_max_number_ != nullptr)
    exit_threshold_max_number_->publish_state(exit->threshold->max_percentage.value_or(0));
  if (roi_height_number_ != nullptr)
    roi_height_number_->publish_state(entry->roi_override->height);
  if (roi_width_number_ != nullptr)
    roi_width_number_->publish_state(entry->roi_override->width);
  if (refresh_mode_select_ != nullptr)
    refresh_mode_select_->publish_state(distanceSensor->is_interrupt_enabled() ? "interrupt" : "polling");
...
void Roode::update() {
  if (invalid_distance_limit_number_ != nullptr) {
    uint8_t val = static_cast<uint8_t>(invalid_distance_limit_number_->state);
    if (val != invalid_distance_limit_)
      invalid_distance_limit_ = val;
  }
  if (restart_timeout_number_ != nullptr) {
    uint32_t val = static_cast<uint32_t>(restart_timeout_number_->state) * 1000;
    if (val != restart_timeout_ms_)
      restart_timeout_ms_ = val;
  }
  if (detection_min_number_ != nullptr) {
    uint8_t val = static_cast<uint8_t>(detection_min_number_->state);
    if (val != entry->threshold->min_percentage.value_or(0)) {
      entry->threshold->min_percentage = val;
      exit->threshold->min_percentage = val;
      if (entry->threshold->idle > 0) {
        entry->threshold->min = (entry->threshold->idle * val) / 100;
        exit->threshold->min = (exit->threshold->idle * val) / 100;
      }
    }
  }
  if (detection_max_number_ != nullptr) {
    uint8_t val = static_cast<uint8_t>(detection_max_number_->state);
    if (val != entry->threshold->max_percentage.value_or(0)) {
      entry->threshold->max_percentage = val;
      exit->threshold->max_percentage = val;
      if (entry->threshold->idle > 0) {
        entry->threshold->max = (entry->threshold->idle * val) / 100;
        exit->threshold->max = (exit->threshold->idle * val) / 100;
      }
    }
  }
  if (filter_mode_select_ != nullptr) {
    std::string val = filter_mode_select_->state;
  ...
  if (log_fallback_switch_ != nullptr)
    log_fallback_events_ = log_fallback_switch_->state;
  if (force_single_core_switch_ != nullptr)
    force_single_core_ = force_single_core_switch_->state;
  if (sampling_number_ != nullptr) {
    uint8_t val = static_cast<uint8_t>(sampling_number_->state);
    if (val != samples)
      set_sampling_size(val);
  }
  if (filter_window_number_ != nullptr) {
    uint8_t val = static_cast<uint8_t>(filter_window_number_->state);
    if (val != filter_window_)
      set_filter_window(val);
  }
  if (calibration_ranging_select_ != nullptr) {
    std::string val = calibration_ranging_select_->state;
    if (val != "auto") {
      const RangingMode *mode = val == "short"   ? vl53l1x::Ranging::Short
                                 : val == "medium" ? vl53l1x::Ranging::Medium
                                                   : vl53l1x::Ranging::Long;
      distanceSensor->set_ranging_mode_override(mode);
    } else {
      distanceSensor->set_ranging_mode_override(nullptr);
    }
  }
  if (calibration_offset_number_ != nullptr)
    distanceSensor->set_offset(static_cast<int16_t>(calibration_offset_number_->state));
  if (calibration_crosstalk_number_ != nullptr)
    distanceSensor->set_xtalk(static_cast<uint16_t>(calibration_crosstalk_number_->state));
  if (calibration_persistence_switch_ != nullptr)
    calibration_persistence_ = calibration_persistence_switch_->state;
  if (zones_invert_switch_ != nullptr)
    invert_direction_ = zones_invert_switch_->state;
  if (entry_roi_height_number_ != nullptr)
    entry->roi_override->height = static_cast<uint8_t>(entry_roi_height_number_->state);
  if (entry_roi_center_number_ != nullptr)
    entry->roi_override->center = static_cast<uint8_t>(entry_roi_center_number_->state);
  if (entry_threshold_min_number_ != nullptr)
  {
    uint8_t val = static_cast<uint8_t>(entry_threshold_min_number_->state);
    entry->threshold->min_percentage = val;
    if (entry->threshold->idle > 0)
      entry->threshold->min = (entry->threshold->idle * val) / 100;
  }
  if (entry_threshold_max_number_ != nullptr)
  {
    uint8_t val = static_cast<uint8_t>(entry_threshold_max_number_->state);
    entry->threshold->max_percentage = val;
    if (entry->threshold->idle > 0)
      entry->threshold->max = (entry->threshold->idle * val) / 100;
  }
  if (exit_roi_height_number_ != nullptr)
    exit->roi_override->height = static_cast<uint8_t>(exit_roi_height_number_->state);
  if (exit_roi_center_number_ != nullptr)
    exit->roi_override->center = static_cast<uint8_t>(exit_roi_center_number_->state);
  if (exit_threshold_min_number_ != nullptr)
  {
    uint8_t val = static_cast<uint8_t>(exit_threshold_min_number_->state);
    exit->threshold->min_percentage = val;
    if (exit->threshold->idle > 0)
      exit->threshold->min = (exit->threshold->idle * val) / 100;
  }
  if (exit_threshold_max_number_ != nullptr)
  {
    uint8_t val = static_cast<uint8_t>(exit_threshold_max_number_->state);
    exit->threshold->max_percentage = val;
    if (exit->threshold->idle > 0)
      exit->threshold->max = (exit->threshold->idle * val) / 100;
  }
  if (roi_height_number_ != nullptr) {
    entry->roi_override->height = static_cast<uint8_t>(roi_height_number_->state);
    exit->roi_override->height = static_cast<uint8_t>(roi_height_number_->state);
  }
  if (roi_width_number_ != nullptr) {
    entry->roi_override->width = static_cast<uint8_t>(roi_width_number_->state);
    exit->roi_override->width = static_cast<uint8_t>(roi_width_number_->state);
  }
  if (refresh_mode_select_ != nullptr) {
    bool use_int = refresh_mode_select_->state == "interrupt";
    distanceSensor->set_interrupt_enabled(use_int);
  }
