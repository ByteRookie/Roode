...
[![GitHub release](https://img.shields.io/badge/release-1.6.2-blue?style=flat-square)](https://github.com/Lyr3x/Roode/releases/tag/1.6.2)
...
roode:
  id: roode_platform
  # Username/password for web portal basic auth
  web_username: admin
  web_password: !secret web_password
  # Optional token required for API access; omit to leave the API open
  portal_password: !secret api_token
...
| `roode.trial_bump_cv` | Optional | `20%` | CV threshold to stop bump trials | Passive scans end too soon or too late | Adjust in ~5% steps; lower for more stability | `trial_bump_cv: 15%` | `trial_bump_cv: 30%` |
| `roode.web_username` & `roode.web_password` | Optional | none | Basic auth credentials for portal | Restrict portal access on shared networks | Set both to enable HTTP basic auth | `web_username: admin`<br>`web_password: !secret web_password` | *(not set)* |
| `roode.portal_password` | Optional | none | Shared token for API requests | Restrict API access on shared networks | Supply via `token` query or `X-Portal-Token` header | `portal_password: !secret api_token` | *(not set)* |
| `roode.scan_time_cap_seconds` | Optional | `90s` | Max duration of passive scan | Scan runs too long or needs more time | Keep within 60–180s as needed | `scan_time_cap_seconds: 120s` | `scan_time_cap_seconds: 60s` |
| `roode.roi_result` | Optional | none | Load ROI/thresholds from file | Using offline calibration results | Provide path to JSON file generated by `roi_analysis.py` | *(not set)* | `roi_result: roi_result.json` |
...
| `variance_cv_mask` | sensor | Coefficient of variation for scan MCPS mask |
| `trial_bump_cv` | sensor | Mean CV of distance across scan trials |
| `scan_time_cap_seconds` | sensor | Elapsed time of passive scan session |
...
2. **Open the calibration portal** at `http://<device>:8080`.
...
The portal starts automatically when the device boots and listens on port 8080.
Access it at `http://<device>:8080` using the credentials from `web_username`/`web_password`.

The following JSON endpoints are served on the main device web server:
...
Calibration follows a passive-scan → recommendation → apply cycle:

1. Start a passive scan with the portal button or `POST /api/scan/start`.
2. When the scan completes, call `/api/roi/preview` to review the suggested ROI and thresholds.
3. Apply them via `/api/roi/apply` to write the settings to flash.

Set `portal_password` in the `roode:` configuration to require a token for API requests. Clients supply it via a `token` query parameter or `X-Portal-Token` header. The web portal itself uses HTTP basic authentication via `web_username` and `web_password`.
...
1. Open the portal at `http://<device>:8080`.
